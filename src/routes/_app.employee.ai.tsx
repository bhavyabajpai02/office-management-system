...
const quickActions: Array<{
...
}> = [
  {
    ...
    prompt: "Generate three quarterly goals for improving customer response efficiency and automation quality.",
    icon: Target,
  },
  {
    mode: "improve",
  ...
  },
  {
    ...
    prompt: "Analyze my quarter and suggest practical next actions based on delayed milestones and approval risk.",
    icon: TrendingUp,
  },
  {
    mode: "summary",
...
export function AIAssistantPage() {
  const { role } = useAuth();
  const [prompt, setPrompt] = useState("Generate three quarterly goals for improving customer response efficiency and automation quality.");
  ...
  const selectedAction = useMemo(() => quickActions.find((action) => action.mode === mode) ?? quickActions[0], [mode]);
  const workspaceLink =
    role === "admin"
      ? "/admin/analytics"
      : role === "manager"
  ...
  return (
    <div className="flex min-h-0 flex-col">
      ...
      <div className="grid min-h-0 gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
        ...
        <div className={`grid min-h-0 gap-4 ${contextOpen ? "lg:grid-cols-[minmax(0,1fr)_360px]" : "lg:grid-cols-[minmax(0,1fr)]"}`}>
          <Card className="flex h-[calc(100vh-8.5rem)] min-h-[620px] flex-col overflow-hidden">
            <div className="flex shrink-0 items-center justify-between border-b bg-muted/35 p-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button size="icon" variant="ghost" onClick={() => setContextOpen((open) => !open)} aria-label="Toggle context panel">
                ...
                <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                  ...
                  >
                    <div className="mb-2 flex items-center gap-2 text-xs opacity-80">
                      {message.role === "assistant" ? <Bot className="h-3.5 w-3.5" /> : <MessageSquareText className="h-3.5 w-3.5" />}
                      {message.role === "assistant" ? "Assistant" : "You"}
                      <span>-</span>
                      <span>{formatTime(message.createdAt)}</span>
                    </div>
                    <p className="leading-6">{message.content}</p>
                    {message.suggestion && <SuggestionCard suggestion={message.suggestion} onCopy={() => copySuggestion(message.suggestion ?? null)} />}
                  </div>
                </div>
              ))}
            ...
            <div className="shrink-0 border-t bg-background p-4">
              <div className="space-y-3">
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div className="text-xs text-muted-foreground">Tip: include the team, metric, timeframe, and business outcome.</div>
                  <div className="flex gap-2">
                    <Button variant="outline" disabled={!lastSuggestion || busy} onClick={() => runAssistant(prompt, mode)}>
          ...
          {contextOpen && <div className="space-y-4 lg:max-h-[calc(100vh-8.5rem)] lg:overflow-y-auto lg:pr-1">
            <SectionCard title="Structured output" description="Ready to paste into a goal sheet.">
              {lastSuggestion ? (
                <div className="space-y-4">
                  <SuggestionCard suggestion={lastSuggestion} compact onCopy={() => copySuggestion(lastSuggestion)} />
                ...
                <div className="rounded-lg border border-dashed p-6 text-center">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">Run an AI action to produce a structured goal.</p>
                </div>
              )}
            </SectionCard>
            ...
            <SectionCard title="Prompt recipes">
              <Tabs defaultValue="goal">
                ...
                {[
                  ["goal", "Turn 'improve onboarding' into a SMART quarterly goal for a product team."],
                  ["kpi", "Suggest leading and lagging KPIs for a customer retention objective."],
                  ["review", "Summarize weekly progress with wins, risks, blockers, and next actions."],
                ].map(([value, text]) => (
  ...
  return (
    <div className={`mt-4 rounded-lg border bg-background/80 p-4 ${compact ? "mt-0" : ""}`}>
      ...
      <div className="mt-4 grid gap-3">
        <div>
          <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">KPIs</div>
          <div className="grid gap-2">
            {suggestion.kpis.map((kpi) => (
              <div key={kpi} className="rounded-md bg-muted/50 px-3 py-2 text-xs">{kpi}</div>
            ))}
          </div>
        </div>
        {!compact && (
          <div>
            <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">Milestones</div>
            <div className="grid gap-2 sm:grid-cols-2">
              {suggestion.milestones.map((milestone) => (
                <div key={milestone} className="rounded-md border bg-card px-3 py-2 text-xs leading-5">{milestone}</div>
              ))}
            </div>
          </div>
        )}
...
function buildSuggestion(prompt: string, mode: AssistantMode): Suggestion {
  ...
  const templates: Record<AssistantMode, Omit<Suggestion, "thrust" | "confidence">> = {
    quarterly: {
      ...
      kpis: ["Primary KPI: 20% improvement against baseline", "Leading KPI: weekly milestone completion above 85%", "Quality KPI: stakeholder satisfaction above 4.5/5"],
      milestones: ["Q1: Confirm baseline and publish owner map", "Q2: Pilot process changes with one team", "Q3: Scale the playbook across the function", "Q4: Hit target and document operating rhythm"],
    },
    improve: {
      ...
      kpis: ["Specific outcome metric with current baseline", "Target threshold due by quarter end", "Review cadence with manager sign-off"],
      milestones: ["Clarify business outcome and target audience", "Define baseline and reporting source", "Add quarterly checkpoints", "Confirm target and approval owner"],
    },
    kpis: {
      ...
      kpis: ["Outcome: target attainment percentage", "Leading: weekly delivery predictability", "Quality: defect or rework rate", "Adoption: stakeholder usage or participation"],
      milestones: ["Select source system", "Set baseline", "Create weekly dashboard", "Review variance and corrective actions"],
    },
    team: {
      ...
      kpis: ["Team progress: weighted average above 80%", "Dependency SLA: blockers resolved within 3 business days", "Engagement: check-in completion above 95%"],
      milestones: ["Map owners and dependencies", "Create shared goal records", "Run midpoint review", "Publish executive summary"],
    },
    insights: {
      ...
      kpis: ["Delayed goal count reduced by 30%", "Approval age below 2 business days", "Check-in completeness above 95%"],
      milestones: ["Identify top three blockers", "Create recovery actions", "Notify owners", "Review trend next week"],
    },
    summary: {
      ...
      kpis: ["Wins: milestones completed", "Risks: delayed dependencies", "Next steps: owner and due date"],
      milestones: ["Summarize wins", "Name blockers", "Request manager decision", "Confirm next update date"],
    },
  };
...
function inferThrust(lower: string) {
  if (lower.includes("customer") || lower.includes("support") || lower.includes("retention")) return "Customer Experience";
  ...
  if (lower.includes("team") || lower.includes("people") || lower.includes("manager")) return "People & Culture";
  return THRUST_AREAS.includes("Digital Transformation") ? "Digital Transformation" : THRUST_AREAS[0];
}
