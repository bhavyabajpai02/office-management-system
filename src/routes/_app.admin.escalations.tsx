...
const demoEscalations: Escalation[] = [
  {
    ...
    severity: "high",
    age: "3d",
    summary: "Five goal sheets waiting past manager SLA.",
    status: "open",
  },
  {
    id: "ESC-1041",
    ...
    severity: "medium",
    age: "2d",
    summary: "Q2 check-in missing for two customer retention goals.",
    status: "watching",
  },
  {
    id: "ESC-1038",
    ...
    severity: "critical",
    age: "6h",
    summary: "Bulk goal status update requires HR review.",
    status: "open",
  },
  {
    id: "ESC-1035",
    ...
    severity: "medium",
    age: "1d",
    summary: "Progress under 55% on shared automation objective.",
    status: "resolved",
  },
];
...
function EscalationsPage() {
  ...
  const filtered = useMemo(() => {
    return rows.filter((row) => {
      ...
      const hay =
        `${row.id} ${row.owner} ${row.department} ${row.type} ${row.summary}`.toLowerCase();
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <>
            <Button variant="outline" onClick={exportEscalations}>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          label="Critical"
          value={criticalCount}
          hint="needs same-day review"
          icon={<AlertTriangle className="h-4 w-4" />}
        />
        <StatCard
          label="Watching"
          value={rows.filter((row) => row.status === "watching").length}
      ...
      <SectionCard>
        <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-center">
          <div className="relative flex-1">
            ...
            <Input
              className="pl-9"
              placeholder="Search owner, department, type, or summary..."
              value={search}
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-2">
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => updateStatus(row.id, "watching")}
                    >
                      Watch
                    </Button>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        {[
          ...
          <Link
            key={title}
            to={href}
            className="rounded-lg border bg-card p-4 transition hover:bg-muted/40"
          >
...
function SeverityBadge({ severity }: { severity: Escalation["severity"] }) {
  ...
  if (severity === "high")
    return <Badge className="bg-warning text-warning-foreground hover:bg-warning">High</Badge>;
