...
function EmployeeDashboard() {
  ...
  const completionPct = total === 0 ? 0 : Math.round(
    goals.reduce((acc, g) => acc + calcProgressForGoal(g), 0) / total
  );
  ...
  const statusBuckets = ["draft","submitted","rework_requested","approved","locked"].map((s) => ({
    name: s,
    value: goals.filter((g) => g.status === s).length,
  })).filter((b) => b.value > 0);

  const COLORS = ["hsl(220 9% 60%)", "var(--info)", "var(--warning)", "var(--success)", "var(--primary)"];
  ...
  return (
    <div>
        ...
        actions={
          <>
            <Button asChild variant="outline">
              <Link to="/ai"><Sparkles className="h-4 w-4 mr-1.5" /> AI Copilot</Link>
            </Button>
            <Button asChild>
              <Link to="/employee/goals"><Target className="h-4 w-4 mr-1.5" /> Manage Goals</Link>
            </Button>
          </>
        }
      />
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        <StatCard label="Total goals" value={total} hint={`out of 8 max`} icon={<Target className="h-4 w-4" />} />
        <StatCard label="Approved" value={approved} hint="locked in for the cycle" icon={<CheckCircle2 className="h-4 w-4" />} />
        <StatCard label="Pending approval" value={pending} hint="awaiting manager" icon={<Clock className="h-4 w-4" />} />
        <StatCard label="Avg. progress" value={`${completionPct}%`} hint="across all goals" icon={<TrendingUp className="h-4 w-4" />} />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3 mb-6">
        <SectionCard title="Goal progress" description="Real-time achievement per goal" >
          <div className="lg:col-span-2 h-64">
            {chartData.length === 0 ? (
              <EmptyState title="No goals yet" description="Create your first goal to see progress." />
            ) : (
        ...
        <SectionCard title="Goal status mix">
          <div className="h-64">
            {statusBuckets.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={statusBuckets} dataKey="value" nameKey="name" innerRadius={50} outerRadius={80} paddingAngle={2}>
                    {statusBuckets.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                  </Pie>
      ...
      <SectionCard title="My goals" description="Snapshot of your goal sheet"
        actions={<Button asChild variant="outline" size="sm"><Link to="/employee/goals">Open</Link></Button>}>
        {isLoading ? (
          <div className="space-y-3">
            {[0,1,2].map((i) => <div key={i} className="h-12 rounded-md bg-muted/40 animate-pulse" />)}
          </div>
        ) : goals.length === 0 ? (
          <EmptyState
      ...
      <div className="grid gap-4 lg:grid-cols-2 mt-6">
        <SectionCard title="Upcoming check-ins">
          <ul className="space-y-2 text-sm">
            {["Q1","Q2","Q3","Q4"].map((q) => (
              <li key={q} className="flex items-center justify-between rounded-md bg-muted/40 px-3 py-2">
                <span className="flex items-center gap-2"><CalendarClock className="h-4 w-4 text-muted-foreground" /> {q} self-review and manager feedback</span>
                <Link to="/employee/checkins" className="text-xs font-medium text-accent hover:underline">Open</Link>
              </li>
            ))}
          </ul>
        </SectionCard>
...
function calcProgressForGoal(g: { uom_direction: string; target: number | string; check_ins?: { actual: number | null }[] }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <span className={`text-sm font-semibold ${ok ? "text-success" : "text-warning-foreground"}`}>{total}%</span>
      </div>
      <Progress value={Math.min(total, 100)} className="h-2" />
      <p className="text-xs text-muted-foreground">
        {ok ? "Your goal sheet is balanced — ready for submission." : "Total weightage must equal 100% before you can submit."}
      </p>
    </div>
  );
}
